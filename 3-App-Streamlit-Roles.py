...
with st.sidebar:
    ...
    if st.session_state.rol is None:
        ...
        # COMBO para fincas: cargar de Google Sheets
        df_fincas_combo, _ = get_df_safe("Fincas")
        if not df_fincas_combo.empty and 'id_finca' in df_fincas_combo.columns:
            # Crear opciones bonitas: "06 - ESTRIBO" pero valor real es ID
            opciones_fincas = []
            for _, r in df_fincas_combo.iterrows():
                ...
                opciones_fincas.append(f"{fid} - {nombre}" if nombre else fid)
            # Deduplicar manteniendo orden
            seen = set()
            opciones_fincas_unique = []
            for o in opciones_fincas:
                fid_norm = norm_id(o.split(' - ')[0])
                if fid_norm not in seen:
                    seen.add(fid_norm)
                    opciones_fincas_unique.append(o)
            opciones_fincas = opciones_fincas_unique
        else:
            opciones_fincas = ["06 - ESTRIBO", "FIN-001 - La Esperanza", "FIN-002 - San Jorge", "FIN-003 - El Rosario"]
        
        if rol in ["VIGILANCIA", "JEFE_PLANTA"]:
            # COMBO en lugar de texto
            finca_combo_sel = st.selectbox("ID Finca (combo)", opciones_fincas, help="Selecciona tu finca del listado")
            id_finca = finca_combo_sel.split(' - ')[0].strip()  # extraer solo ID
            # También permitir escribir si no está en lista
            if st.checkbox("Mi finca no está en lista, escribir manual"):
                id_finca = st.text_input("ID Finca manual (TEXTO) Ej: 06", value=id_finca)
        else:
            id_finca = st.text_input("ID Finca (Obligatorio Vigilancia/Planta) Ej: 06 o FIN-001", value="OFICINA")
            finca_combo_sel = id_finca
            
        password = st.text_input("Contraseña", type="password", help="Vigilancia: clave = ID finca o Banano2026. Oficina: admin123 o Banano2026")
        ...
        if st.button("Entrar"):
            ...
            if rol in ["VIGILANCIA", "JEFE_PLANTA"] and not id_finca:
                st.error("❌ Debes seleccionar ID Finca")
            elif rol in ["VIGILANCIA", "JEFE_PLANTA"] and not password:
                st.error("❌ Debes poner contraseña")
            else:
                finca_valida = True
                if rol in ["VIGILANCIA", "JEFE_PLANTA"]:
                    df_fincas_check = df_fincas_combo
                    if not df_fincas_check.empty and id_finca:
                        existe = df_fincas_check['id_finca'].astype(str).apply(norm_id).eq(norm_id(id_finca)).any()
                        if not existe and id_finca.upper() != "TODAS" and password != CLAVE_MAESTRA:
                            if not id_finca.isdigit():
                                st.error(f"❌ Finca {id_finca} no existe. Pide alta a Oficina")
                                finca_valida = False
                    if finca_valida:
                        clave_esperada = CLAVES_FINCAS.get(id_finca) or CLAVES_FINCAS.get(id_finca.upper()) or CLAVES_FINCAS.get(norm_id(id_finca))
                        if password == CLAVE_MAESTRA or password == id_finca or password == norm_id(id_finca) or (clave_esperada and password == clave_esperada):
                    ...
                    if password == CLAVE_MAESTRA or password == CLAVES_FINCAS.get("OFICINA_CENTRAL"):
                        ...
                        st.error("❌ Contraseña incorrecta Oficina: admin123 o Banano2026")
    else:
