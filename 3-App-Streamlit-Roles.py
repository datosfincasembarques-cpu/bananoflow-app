...
"""
3 - APP BANANO FLOW - V4 FINAL - IDs TEXTO, forms limpios, vigilancia auto por finca, solo botones, seguridad
"""
...
with st.sidebar:
    ...
    if st.session_state.rol is None:
        rol = st.selectbox("Rol", ROLES, help="OFICINA_CENTRAL / VIGILANCIA / JEFE_PLANTA")
        
        # Cargar usuarios y fincas para combos
        df_fincas_combo, _ = get_df_safe("Fincas")
        df_usuarios_combo, _ = get_df_safe("Usuarios")
        
        # Si no hay hoja Usuarios, crear lista base
        if df_usuarios_combo.empty:
            # Usuarios base + fincas como usuarios
            usuarios_base = [
                ...
                {"usuario": "vigilancia_FIN-001", "nombre": "Vigilancia FIN-001", "rol": "VIGILANCIA", "id_finca": "FIN-001", "password": "finca001"},
                {"usuario": "planta_06", "nombre": "Jefe Planta 06", "rol": "JEFE_PLANTA", "id_finca": "06", "password": "planta123"},
            ]
            # Agregar fincas como usuarios si existen
            if not df_fincas_combo.empty:
                for _, r in df_fincas_combo.iterrows():
                    fid = str(r.get('id_finca','')).strip()
                    if not fid or fid.lower()=='nan':
                        continue
                    nom = str(r.get('nombre','')).strip()
                    if fid not in [u["id_finca"] for u in usuarios_base if u["rol"]=="VIGILANCIA"]:
                        usuarios_base.append({"usuario": f"vigilancia_{fid}", "nombre": f"Vigilancia {nom or fid}", "rol": "VIGILANCIA", "id_finca": fid, "password": fid})
            df_usuarios_combo = pd.DataFrame(usuarios_base, dtype=str)
        
        # Filtrar usuarios por rol seleccionado
        if not df_usuarios_combo.empty and 'rol' in df_usuarios_combo.columns:
            df_usuarios_filtrados = df_usuarios_combo[df_usuarios_combo['rol'].astype(str).str.upper() == rol.upper()]
            if df_usuarios_filtrados.empty:
                df_usuarios_filtrados = df_usuarios_combo
        else:
            df_usuarios_filtrados = df_usuarios_combo
        
        # Crear opciones para combo de usuarios: "usuario - Nombre (Finca)"
        opciones_usuarios = []
        if not df_usuarios_filtrados.empty:
            for _, r in df_usuarios_filtrados.iterrows():
                user = str(r.get('usuario','')).strip()
                nombre = str(r.get('nombre', r.get('nombre_completo',''))).strip()
                id_finca_u = str(r.get('id_finca','')).strip()
                label = f"{user} - {nombre}" if nombre else user
                if id_finca_u and id_finca_u.lower()!='nan':
                    label += f" ({id_finca_u})"
                opciones_usuarios.append(label)
        
        if not opciones_usuarios:
            opciones_usuarios = ["admin - Oficina Central (OFICINA)", "vigilancia_06 - Vigilancia ESTRIBO (06)"]
        
        # COMBO 2: Usuario
        usuario_combo_sel = st.selectbox("Usuario", opciones_usuarios, help="Selecciona tu usuario")
        # Extraer usuario real (antes del primer " - ")
        usuario_id = usuario_combo_sel.split(' - ')[0].strip()
        # Buscar datos de ese usuario
        datos_usuario = df_usuarios_filtrados[df_usuarios_filtrados['usuario'].astype(str)==usuario_id] if not df_usuarios_filtrados.empty else pd.DataFrame()
        if not datos_usuario.empty:
            id_finca = str(datos_usuario.iloc[0].get('id_finca','')).strip()
            nombre_usuario = str(datos_usuario.iloc[0].get('nombre','')).strip()
            rol_real = str(datos_usuario.iloc[0].get('rol', rol)).strip()
            password_esperada = str(datos_usuario.iloc[0].get('password','')).strip()
        else:
            id_finca = "OFICINA"
            nombre_usuario = usuario_id
            rol_real = rol
            password_esperada = ""
            # Fallback a fincas combo si no hay usuario
            if not df_fincas_combo.empty:
                # intentar extraer finca del label
                if '(' in usuario_combo_sel and ')' in usuario_combo_sel:
                    id_finca = usuario_combo_sel.split('(')[-1].replace(')','').strip()
        
        # Mostrar info de finca detectada
        if rol in ["VIGILANCIA", "JEFE_PLANTA"]:
            st.caption(f"Finca detectada: {id_finca}")
        ...
        CLAVES_FINCAS = {
            "06": "06", "6": "06",
            "FIN-001": "finca001", "FIN-002": "finca002", "FIN-003": "finca003",
        ...
        if st.button("Entrar"):
            if not usuario_id:
                ...
                st.error("❌ Pon contraseña")
            else:
                ok = False
                # Validar contra password de hoja Usuarios si existe
                if password_esperada and password == password_esperada:
                    ok = True
                # Validar contra claves fincas
                elif password == CLAVE_MAESTRA:
                    ok = True
                elif password == id_finca or password == norm_id(id_finca):
                    ok = True
                else:
                    clave_esperada = CLAVES_FINCAS.get(id_finca) or CLAVES_FINCAS.get(id_finca.upper()) or CLAVES_FINCAS.get(norm_id(id_finca)) or CLAVES_FINCAS.get(rol_real)
                    if clave_esperada and password == clave_esperada:
                        ok = True
                
                if ok:
                    st.session_state.rol = rol_real if rol_real else rol
...
if st.session_state.rol == "OFICINA_CENTRAL":
    ...
    st.title(f"🚧 Vigilancia - Finca {st.session_state.id_finca}")
    ...
    # UN SOLO FILTRO: por finca normalizado (06==6)
    ...
    if not df_pendientes.empty:
        st.subheader(f"🚛 Órdenes PENDIENTES para {id_finca_vig} (filtro único: finca)")
