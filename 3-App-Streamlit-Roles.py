...
if st.session_state.rol == "OFICINA_CENTRAL":
    ...
    with tab2:
        st.subheader("Guias Fitosanitarias - Stock (R,E,P,D4,D5)")
    ...
    # TAB EN FINCA PARA SALIDA - FOTO DE SALIDA DEL CARRO CARGADO (ya no placas)
    if not df_en_finca.empty:
        st.divider()
        st.subheader(f"🚪 SALIDA DE FINCA {id_finca_vig} - Foto del carro saliendo cargado")
        st.info("Acordamos: En salida ya NO es foto de placas, es foto de donde va saliendo el carro ya cargado")
        ...
        for idx, row in df_en_finca.iterrows():
            ...
            with st.container(border=True):
                st.markdown(f"### 🚛 {id_orden} - LISTO PARA SALIR DE {row['id_finca']}")
                col1, col2 = st.columns([1,1])
                with col1:
                    st.write(f"**Op:** {op_det} | **Caja1:** {caja1_id} | **Caja2:** {caja2_id if caja2_id else 'Sencillo'}")
                    st.write(f"**Visita No:** {row.get('orden_visita','')} | **Entrada:** {row.get('estado_carga','EN_FINCA')}")
                    st.caption("Este carro ya fue cargado en planta, ahora registra su salida de la finca")
                with col2:
                    foto_salida_carro = st.file_uploader(f"📸 Foto SALIDA - Carro saliendo cargado {id_orden}", type=["jpg","jpeg","png"], key=f"f_salida_{id_orden}_{idx}", help="Foto del tracto + caja saliendo por la puerta, ya cargado")
                    odometro_sal = st.text_input(f"Odómetro Salida {id_orden}", key=f"odo_sal_{id_orden}_{idx}", placeholder="Ej: 125600")
                    obs_sal = st.text_area(f"Observaciones Salida {id_orden}", key=f"obs_sal_{id_orden}_{idx}", placeholder="¿Salio completo? ¿Cuantas cajas? ¿Alguna novedad?")
                    es_full = st.checkbox(f"¿Es Full con 2 cajas? {id_orden}", key=f"full_sal_{id_orden}_{idx}")
                    foto_salida_carro2 = None
                    if es_full:
                        foto_salida_carro2 = st.file_uploader(f"📸 Foto SALIDA 2da Caja {id_orden} (Full)", type=["jpg","jpeg","png"], key=f"f_salida2_{id_orden}_{idx}")

                if st.button(f"✅ REGISTRAR SALIDA - CARRO SALIENDO - {id_orden}", key=f"btn_sal_{id_orden}_{idx}", type="primary", use_container_width=True):
                    if not foto_salida_carro:
                        st.error("⚠️ Toma la foto de salida del carro cargado (obligatoria)")
                    else:
                        with st.spinner(f"Subiendo foto salida {id_orden}..."):
                            link_salida = subir_foto_a_drive(foto_salida_carro, f"SALIDA_CARGADO_{id_orden}_{row['id_finca']}_{datetime.now().strftime('%H%M%S')}.jpg")
                            link_salida2 = subir_foto_a_drive(foto_salida_carro2, f"SALIDA_CARGADO2_{id_orden}_{row['id_finca']}_{datetime.now().strftime('%H%M%S')}.jpg") if foto_salida_carro2 else ""
                            fotos_links_s = f"Salida Cargado:{link_salida}"
                            if link_salida2:
                                fotos_links_s += f"|Salida Cargado2:{link_salida2}"
                        ...
                        try:
                            ...
                            append_row_dict_safe(ws_bit, {
                                ...
                                "observaciones": f"Salida carro cargado: {obs_sal} - Op {op_det}",
                                "id_usuario": st.session_state.username,
                            ...
                            st.success(f"✅ Salida {id_orden} registrada - Foto salida: {link_salida}")
                            st.balloons()
                            st.rerun()
                        except Exception as e:
