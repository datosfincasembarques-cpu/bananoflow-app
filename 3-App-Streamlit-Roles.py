...
if st.session_state.rol == "OFICINA_CENTRAL":
    ...
    with tab1:
        ...
        # --- DASHBOARD INFORMATIVO DE STOCK DE GUIAS SANITARIAS (LOTES ABIERTOS) ---
        st.markdown("### 📊 STOCK DE GUIAS SANITARIAS")
        df_stock_guias, _ = get_df_safe("Guias_Folios_Stock")
        if not df_stock_guias.empty:
            try:
                col_estado_lote = next((c for c in df_stock_guias.columns if 'estado_lote' in c.lower() or 'lote_estado' in c.lower()), None)
                col_estado = next((c for c in df_stock_guias.columns if c.lower() == 'estado'), None)
                col_empresa = next((c for c in df_stock_guias.columns if 'empresa' in c.lower() or 'origen' in c.lower()), df_stock_guias.columns[0])
                col_lote = next((c for c in df_stock_guias.columns if 'lote' in c.lower()), None)

                df_abiertos = df_stock_guias.copy()
                if col_estado_lote:
                    df_abiertos = df_abiertos[df_abiertos[col_estado_lote].astype(str).str.upper().isin(['ABIERTO', 'ACTIVO', 'ABIERTA'])]
                
                if col_lote and not df_abiertos.empty:
                    grupos = df_abiertos.groupby([col_empresa, col_lote])
                    cols_dash = st.columns(min(max(len(grupos), 1), 3))
                    idx_col = 0
                    for (empresa, lote), grp in grupos:
                        disp = len(grp[grp[col_estado].astype(str).str.upper().isin(['DISPONIBLE', 'DISP', 'ACTIVO', 'LIBRE'])]) if col_estado else len(grp)
                        util = len(grp[grp[col_estado].astype(str).str.upper().isin(['UTILIZADA', 'USADA', 'CONSUMIDA', 'OCUPADA'])]) if col_estado else 0
                        
                        with cols_dash[idx_col % len(cols_dash)]:
                            with st.container(border=True):
                                st.markdown(f"**{empresa}**")
                                st.caption(f"Lote: {lote}")
                                c_m1, c_m2 = st.columns(2)
                                c_m1.metric("DISPONIBLES", disp)
                                c_m2.metric("UTILIZADAS", util)
                        idx_col += 1
                else:
                    st.info("Mostrando resumen general de guías para lotes abiertos.")
            except Exception as e:
                st.warning(f"Error al cargar el dashboard de guías: {e}")
        else:
            st.info("No hay stock de guías registrado.")
