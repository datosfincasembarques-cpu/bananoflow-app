...
if st.session_state.rol == "OFICINA_CENTRAL":
    ...
    # Normalizar ID: 06 == 6, quitar espacios
    def norm_id(x):
        s = str(x).strip()
        # Si es numerico, quitar ceros a la izquierda para comparar pero mantener original
        if s.isdigit():
    ...
    # Filtrar por finca: comparar normalizado
    ...
    if df_filtrada.empty:
        ...
        with st.expander("Ver todas las órdenes (debug)"):
    ...
    # Eliminar filas sin id_orden (cuadros vacíos)
    df_pendientes = df_pendientes[df_pendientes['id_orden'].astype(str).str.strip() != ""]
    df_en_finca = df_en_finca[df_en_finca['id_orden'].astype(str).str.strip() != ""]
    ...
    if not df_pendientes.empty:
        ...
        for idx, row in df_pendientes.iterrows():
            ...
            if not info_orden.empty:
                operador = info_orden.iloc[0].get('id_operador','?')
                tractor = info_orden.iloc[0].get('id_tractor','?')
                caja1 = info_orden.iloc[0].get('id_caja1','?')
            else:
                operador = "?"
                tractor = "?"
                caja1 = "?"
            ...
            with st.container(border=True):
                ...
                with col_btn1:
                    if st.button(f"✅ ENTRADA", key=f"ent_{id_orden}_{row['id_finca']}_{idx}_{id_norm_vig}", type="primary", use_container_width=True):
                        try:
                            try:
        ...
        st.info(f"No hay pendientes para {id_finca_vig} - nada por hacer")
    ...
    if not df_en_finca.empty:
        ...
        for idx, row in df_en_finca.iterrows():
            ...
            with st.container(border=True):
                ...
                with col_btn:
                    if st.button(f"✅ SALIDA", key=f"sal_en_{id_orden}_{row['id_finca']}_{idx}_{id_norm_vig}", type="secondary", use_container_width=True):
                        try:
                            try:
                                ws_bit = sh.worksheet("Bitacora_Vigilancia")
                            except:
