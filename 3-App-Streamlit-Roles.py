...
with st.sidebar:
    ...
    if st.session_state.rol is None:
        ...
        df_fincas = lower_cols(df_fincas_raw)
        df_usuarios = lower_cols(df_usuarios_raw)
        ...
        col_user = find_col(df_usuarios, ["usuario","user","id_usuario","codigo","id"])
        col_nombre = find_col(df_usuarios, ["nombre","nombre_completo","nombres"])
        col_rol = find_col(df_usuarios, ["rol","role","perfil"])
        col_finca = find_col(df_usuarios, ["id_finca","finca","id finca"])
        col_pass = find_col(df_usuarios, ["password","contrasena","contraseña","clave","pass"])

        # Si no hay Usuarios, crear base desde Fincas
        if df_usuarios.empty:
            base = []
            base.append({"usuario":"admin","nombre":"Oficina Central","rol":"OFICINA_CENTRAL","id_finca":"OFICINA","password":"admin123"})
            base.append({"usuario":"martin_gomez","nombre":"Martin Gomez - Oficina Central","rol":"OFICINA_CENTRAL","id_finca":"OFICINA","password":"admin123"})
            if not df_fincas.empty:
                col_f_id = find_col(df_fincas, ["id_finca","finca","id"])
                col_f_nom = find_col(df_fincas, ["nombre","nombre_finca"])
                for _, r in df_fincas.iterrows():
                    fid = str(r.get(col_f_id,"")).strip() if col_f_id else ""
                    if not fid or fid.lower()=="nan":
                        continue
                    fnom = str(r.get(col_f_nom,"")).strip() if col_f_nom else fid
                    # evitar duplicados
                    if fid not in [b["id_finca"] for b in base if b["rol"]=="VIGILANCIA"]:
                        base.append({"usuario": f"vigilancia_{fid}","nombre": f"Vigilancia {fnom}","rol":"VIGILANCIA","id_finca":fid,"password":fid})
                        base.append({"usuario": f"planta_{fid}","nombre": f"Jefe Planta {fnom}","rol":"JEFE_PLANTA","id_finca":fid,"password":"planta123"})
            df_usuarios = pd.DataFrame(base, dtype=str)
            col_user="usuario"; col_nombre="nombre"; col_rol="rol"; col_finca="id_finca"; col_pass="password"
        else:
            ...
            if col_nombre is None:
                df_usuarios["nombre"] = df_usuarios[col_user].astype(str)
                col_nombre="nombre"
            if col_rol is None:
                df_usuarios["rol"] = rol
                col_rol="rol"
            if col_finca is None:
        ...
        # Filtrar por rol
        try:
            df_filt = df_usuarios[df_usuarios[col_rol].astype(str).str.upper() == rol.upper()]
            if df_filt.empty:
                df_filt = df_usuarios
        except:
            df_filt = df_usuarios
        ...
        for _, r in df_filt.iterrows():
            u = str(r.get(col_user,"")).strip()
            if not u or u.lower()=="nan":
                continue
            nom = str(r.get(col_nombre,"")).strip()
            fin = str(r.get(col_finca,"")).strip()
            label = f"{u} - {nom}" if nom and nom!=u else u
            if fin and fin.lower()!="nan":
                label = f"{label} ({fin})"
            # Evitar labels que empiezan con " - "
        ...
        if r_sel is not None:
            usuario_id = str(r_sel.get(col_user,"")).strip()
            id_finca = str(r_sel.get(col_finca,"")).strip()
            nombre_usuario = str(r_sel.get(col_nombre,"")).strip()
            rol_real = str(r_sel.get(col_rol,rol)).strip()
            pass_esperada = str(r_sel.get(col_pass,"")).strip()
        else:
            # parse fallback
        ...
        if rol in ["VIGILANCIA","JEFE_PLANTA"]:
            st.caption(f"Finca: {id_finca}")

        password = st.text_input("Contraseña", type="password")

        CLAVE_MAESTRA="Banano2026"
        CLAVES={"06":"06","6":"06","FIN-001":"finca001","OFICINA_CENTRAL":"admin123","JEFE_PLANTA":"planta123"}

        if st.button("Entrar"):
            if not usuario_id:
                st.error("Selecciona usuario")
            elif not password:
                st.error("Pon contraseña")
            else:
                ok=False
                if pass_esperada and password==pass_esperada:
                    ok=True
                    ...
                    ce = CLAVES.get(id_finca) or CLAVES.get(id_finca.upper() if id_finca else "") or CLAVES.get(norm_id(id_finca) if id_finca else "") or CLAVES.get(rol_real)
                    if ce and password==ce:
                        ok=True
                ...
                if ok:
                    ...
                    st.error(f"Contraseña incorrecta para {usuario_id}")
    else:
